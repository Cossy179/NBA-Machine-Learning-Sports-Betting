"""
XGBoost Model Training - Moneyline Prediction (Research-Based)
...
Research shows calibration-based model selection yields +34.69% ROI vs -35.17% for accuracy-based.
"""
import sqlite3
import sys
import os
...
print("="*70)
print("XGBoost Moneyline Prediction Training - Research-Based Approach")
print("="*70)
...
# Calculate temporal weights (prioritize recent seasons)
...
# Create time-based train/val/test splits (no look-ahead bias)
...
# Optimize hyperparameters using Bayesian optimization with time-based CV
optimizer = XGBoostOptimizer(
    n_trials=100,
    cv_folds=3,
    optimization_metric='composite',  # Prioritizes calibration + performance
    verbose=True
)
...
results_uncal = evaluator.evaluate_model(
    ...
    model_name="XGBoost-Uncalibrated"
)
...
evaluator.plot_reliability_diagram(
    y_test, y_test_pred_uncal,
    title="XGBoost Uncalibrated - Reliability Diagram",
    save_path=f"../../Models/XGBoost_Models/reliability_uncalibrated_{timestamp}.png",
    show=False
)

evaluator.plot_reliability_diagram(
    y_test, y_test_pred_cal,
    title="XGBoost Calibrated - Reliability Diagram",
    save_path=f"../../Models/XGBoost_Models/reliability_calibrated_{timestamp}.png",
    show=False
)

evaluator.plot_confidence_histogram(
    y_test_pred_cal,
    title="XGBoost Calibrated - Prediction Confidence",
    save_path=f"../../Models/XGBoost_Models/confidence_histogram_{timestamp}.png",
    show=False
)
...
# Save base model
model_path = f"{model_dir}/xgboost_ml_calibrated_{timestamp}.pkl"
joblib.dump({
    'model': final_model,
...
# Save metadata file
metadata_path = f"{model_dir}/xgboost_ml_metadata_{timestamp}.txt"
with open(metadata_path, 'w') as f:
    f.write("XGBoost Moneyline Model - Research-Based Training\n")
    f.write("="*60 + "\n\n")
