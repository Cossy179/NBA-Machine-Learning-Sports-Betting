"""
Logistic Regression Model Training - Moneyline Prediction (Research-Based)
...
Logistic regression provides a strong baseline and is inherently well-calibrated.
"""
import sqlite3
import sys
import os
...
print("="*70)
print("Logistic Regression Moneyline Prediction - Research-Based Approach")
print("="*70)
...
results_uncal = evaluator.evaluate_model(
    ...
    model_name="LogisticRegression-Uncalibrated"
)
...
results_cal = evaluator.evaluate_model(
    ...
    model_name="LogisticRegression-Calibrated"
)
...
evaluator.plot_reliability_diagram(
    y_test, y_test_pred_uncal,
    title="Logistic Regression Uncalibrated - Reliability Diagram",
    save_path=f"{model_dir}/logreg_ml_reliability_uncalibrated_{timestamp}.png",
    show=False
)

evaluator.plot_reliability_diagram(
    y_test, y_test_pred_cal,
    title="Logistic Regression Calibrated - Reliability Diagram",
    save_path=f"{model_dir}/logreg_ml_reliability_calibrated_{timestamp}.png",
    show=False
)
...
model_path = f"{model_dir}/logistic_regression_ml_calibrated_{timestamp}.pkl"
joblib.dump({
    'model': final_model,
