"""
Neural Network Model Training - Over/Under Prediction (Research-Based)
...
print("="*70)
print("Neural Network Over/Under Prediction - Research-Based Approach")
print("="*70)
...
results_uncal = evaluator.evaluate_model(
    ...
    model_name="NeuralNetwork-UO-Uncalibrated"
)
...
results_cal = evaluator.evaluate_model(
    ...
    model_name="NeuralNetwork-UO-Calibrated"
)
...
evaluator.plot_reliability_diagram(
    y_test, y_test_pred_uncal,
    title="Neural Network O/U Uncalibrated - Reliability Diagram",
    save_path=f"{model_dir}/nn_ou_reliability_uncalibrated_{timestamp}.png",
    show=False
)

evaluator.plot_reliability_diagram(
    y_test, y_test_pred_cal,
    title="Neural Network O/U Calibrated - Reliability Diagram",
    save_path=f"{model_dir}/nn_ou_reliability_calibrated_{timestamp}.png",
    show=False
)
...
# Save Keras model
keras_model_path = f"{model_dir}/nn_ou_model_{timestamp}.h5"
final_model.save(keras_model_path)
print(f"✅ Keras model saved: {keras_model_path}")

# Save calibrator and metadata
metadata_path = f"{model_dir}/nn_ou_calibrated_{timestamp}.pkl"
joblib.dump({
